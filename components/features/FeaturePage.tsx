...
const overviewDescriptions: Record<FeatureSlug, string> = {
  "safety-score": "Turn your savings, income, spending, and risks into one clear number from 0 to 100, with the few next steps that matter most.",
  monitoring: "We re-check your plan every month and give you a short, plain-English heads-up only when something actually needs your attention.",
  "ai-coach": "Ask your retirement questions in plain English and get clear answers, with every number backed by real calculations — not guesswork.",
  "medicare-social-security": "Understand claiming timing and the Medicare income traps that can quietly raise your costs, in plain dollars.",
  "scam-shield": "Spot the common fraud patterns early and get simple words to slow down any suspicious call, text, or email.",
};

export function FeaturesOverview() {
  return <main><section className="bg-surface py-16 sm:py-20 lg:py-24"><Container className="text-center"><Eyebrow>Features</Eyebrow><h1 className="mx-auto mt-4 max-w-4xl text-4xl font-extrabold tracking-tight text-ink sm:text-5xl lg:text-6xl">Five pillars for a safer, clearer retirement.</h1><p className="mx-auto mt-6 max-w-3xl text-xl leading-9 text-slate-700">Everything in RetireShield works together to answer one question: is your retirement going to be okay? Start with your free Safety Score, then go deeper into monitoring, the AI Coach, Medicare and Social Security timing, and scam protection.</p><div className="mt-8"><Button href="/quiz">Get my free Safety Score</Button></div></Container></section><section className="py-14 sm:py-18 lg:py-20"><Container><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{featureOrder.map((slug) => { const f = features[slug]; const Icon = f.icon; return <Link key={slug} href={`/features/${slug}`} className="group rounded-3xl border border-slate-200 bg-white p-6 no-underline shadow-sm transition hover:-translate-y-1 hover:border-brand/30 hover:shadow-lg"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand/10 text-brand group-hover:bg-brand group-hover:text-white"><Icon className="h-7 w-7" /></div><h2 className="mt-6 text-2xl font-extrabold text-ink">{f.eyebrow}</h2><p className="mt-3 leading-7 text-slate-700">{overviewDescriptions[slug]}</p><span className="mt-5 inline-flex font-extrabold text-brand">Explore {f.eyebrow} →</span></Link>; })}</div></Container></section><SectionBand><Container className="text-center"><h2 className="text-3xl font-extrabold tracking-tight text-ink sm:text-4xl">Start with the score, then go deeper.</h2><p className="mx-auto mt-4 max-w-2xl text-lg leading-8 text-slate-700">It&apos;s free, takes about two minutes, and never asks you to link a bank. See where you stand, then explore whatever matters most to you.</p><div className="mt-8"><Button href="/quiz">Get my free Safety Score</Button></div></Container></SectionBand></main>;
}
