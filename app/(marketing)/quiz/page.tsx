  ...
  if (!introComplete) {
    return (
      <div className="rg-page-shell">
        <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-3xl items-center px-4 py-6 sm:py-10 lg:max-w-5xl">
          <section className="w-full overflow-hidden rounded-[2rem] bg-brand-dark px-5 py-8 text-center text-white shadow-2xl shadow-brand-dark/20 sm:px-10 sm:py-12">
            ...
            <h1 className="mx-auto mt-6 max-w-2xl text-4xl font-bold leading-tight tracking-tight text-white sm:text-5xl lg:max-w-3xl">Take the <span className="text-[#7BD3A8]">free quiz</span> and see <span className="text-[#7BD3A8]">how solid</span> your retirement really is.</h1>
            <p className="mx-auto mt-6 flex max-w-xl items-start justify-center gap-3 text-center text-lg font-semibold leading-8 text-white/90 sm:text-xl lg:max-w-2xl"><svg viewBox="0 0 24 24" fill="none" className="mt-1 size-6 shrink-0 sm:size-7" aria-hidden="true"><circle cx="12" cy="12" r="9" stroke="#7BD3A8" strokeWidth="2" /><path d="M8.5 12.5l2.5 2.5 4.5-5.5" stroke="#7BD3A8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg><span>Real numbers, no sales pitch — just a clear picture of where you stand.</span></p>
            ...
            <div className="mx-auto mt-5 max-w-2xl space-y-2 text-sm font-semibold leading-6 text-[#9FBBDA] sm:text-base"><p>Are you ahead, on track, or behind? Most people are only guessing.</p>{process.env.NEXT_PUBLIC_SOCIAL_PROOF ? <p>{process.env.NEXT_PUBLIC_SOCIAL_PROOF}</p> : null}</div>
          </section>
        </div>
      </div>
    );
